...
def parse_gemini_json(response_text):
    try:
        ...
        st.error(f"❌ Error parsing Gemini JSON: {str(e)}")
        return {}
...
if submit1:
    ...
    if uploaded_file is not None:
        with st.spinner(f"🔍 Calculating ATS match using {company} template..."):
            ...
            Compare the resume with the job description below and return ONLY JSON.

            Job Description:
            {input_text}

            Resume Content (image or extracted text):
            {pdf_content}
            ...
            {{
              ...
              "keywords": {{"Python": 10, "SQL": 7}},
              "sections": {{"Contact Info": true, "Skills": true, "Experience": true, "Projects": false}}
            }}
            """

            response = get_gemini_response(structured_prompt, pdf_content, input_text)
            st.subheader("📊 Raw Gemini Response")
            st.write(response)

            data = parse_gemini_json(response)
            if not data:
                st.warning("⚠️ Could not parse ATS results. Showing raw response.")
            else:
                ...
                df = pd.DataFrame(list(skills.items()), columns=["Type", "Count"])
                fig = px.bar(df, x="Type", y="Count", color="Type", title="Skill Match Analysis")
                st.plotly_chart(fig)

                keywords = data.get("keywords", {})
                if keywords:
                    df_kw = pd.DataFrame(list(keywords.items()), columns=["Keyword", "Count"])
                    fig_kw = px.bar(df_kw, x="Keyword", y="Count", title="Top Keywords in Resume")
                    st.plotly_chart(fig_kw)

                st.subheader("Resume Section Check")
                for section, status in data.get("sections", {}).items():
                    st.write(f"✅ {section}" if status else f"❌ {section}")
    else:
        st.warning("⚠️ Please upload the resume.")
